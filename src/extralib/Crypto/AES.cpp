...
void AES_Crypto::copy_n_bytes (byte * d, byte * s, byte nn)
...
byte AES_Crypto::encrypt (byte plain [N_BLOCK], byte cipher [N_BLOCK])
{
  if (round)
  {
    byte s1 [N_BLOCK], r ;
...
byte AES_Crypto::cbc_encrypt (byte * plain, byte * cipher, int n_block, byte iv [N_BLOCK])
{
  while (n_block--)
  {
    xor_block (iv, plain) ;
...
byte AES_Crypto::cbc_encrypt (byte * plain, byte * cipher, int n_block)
{
  while (n_block--)
  {
    xor_block (iv, plain) ;
...
byte AES_Crypto::decrypt (byte plain [N_BLOCK], byte cipher [N_BLOCK])
{
  if (round)
  {
    byte s1 [N_BLOCK] ;
...
byte AES_Crypto::cbc_decrypt (byte * cipher, byte * plain, int n_block, byte iv [N_BLOCK])
{
  while (n_block--)
  {
    byte tmp [N_BLOCK] ;
...
byte AES_Crypto::cbc_decrypt (byte * cipher, byte * plain, int n_block)
{
  while (n_block--)
  {
    byte tmp [N_BLOCK] ;
...
void AES_Crypto::set_IV(unsigned long long int IVCl)
{
  memcpy(iv, &IVCl, 8);
  memcpy(iv + 8, &IVCl, 8);
  IVC = IVCl;
...
void AES_Crypto::set_size(int sizel)
{
  size = sizel;
}
...
void AES_Crypto::get_IV(byte *out)
{
  memcpy(out, &IVC, 8);
  memcpy(out + 8, &IVC, 8);
}
...
void AES_Crypto::calc_size_n_pad(int p_size)
{
  int s_of_p = p_size - 1;
...
bool AES_Crypto::CheckPad(byte* in, int lsize)
{
  if (in[lsize - 1] <= 0x0f)
  {
...
void AES_Crypto::printArray(byte output[], bool p_pad)
{
  uint8_t i, j;
...
void AES_Crypto::printArray(byte output[], int sizel)
{
...
void AES_Crypto::do_aes_encrypt(byte *plain, int size_p, byte *cipher, byte *key, int bits, byte ivl [N_BLOCK])
{
...
void AES_Crypto::do_aes_encrypt(byte *plain, int size_p, byte *cipher, byte *key, int bits)
{
  calc_size_n_pad(size_p);
  byte plain_p[get_size()];
  padPlaintext(plain, plain_p);
...
void AES_Crypto::do_aes_decrypt(byte *cipher, int size_c, byte *plain, byte *key, int bits, byte ivl [N_BLOCK])
{
  set_size(size_c);
...
void AES_Crypto::do_aes_decrypt(byte *cipher, int size_c, byte *plain, byte *key, int bits)
{
  set_size(size_c);
  int blocks = size_c / N_BLOCK;
  set_key (key, bits);
