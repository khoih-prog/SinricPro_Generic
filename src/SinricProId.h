...
#else    
    void copy(const SinricProId &other) { memcpy(_data._charData, other._data._charData, sizeof(_data._charData)); }    
    void copy(const T &other) { memcpy(_data, other, sizeof(_data)); }
    void copy(const uint8_t other[], size_t size) { memcpy(_data._charData, other, min(sizeof(_data._charData), size)); }
    bool compare(const SinricProId &other) const { return memcmp(_data._charData, other._data._charData, sizeof(_data._charData)) == 0;}  
#endif
